...
class Homeworks(ct.CTkScrollableFrame):
    ...
    def add_item(self, homework_name, course_name, deadline, course_id, item_source_type, item_source_id):
        deadline = datetime.strptime(deadline + '+0000', '%Y-%m-%dT%H:%M:%S.%fZ%z').astimezone(
        ...
        hw_name_label = ct.CTkLabel(hw_frame, text=homework_name).grid(
            row=0, column=0, sticky="ew")
        course_name_label = ct.CTkLabel(hw_frame, text=course_name).grid(
        ...
        hw_outframe.grid(row=len(self.hw_outframes),
                         padx=1, pady=(0, 3), sticky="nsew")
        self.hw_outframes.append(hw_outframe)

    def no_homeworks(self):
        outframe = ct.CTkFrame(self, width=self.winfo_width())
        ct.CTkLabel(outframe, text='現在，課題はありません．', width=outframe.winfo_width()).grid(
